<header>
    <div class="toggle_btn">
        <span></span>
    </div>

    <div class="cercle_btn">
    </div>

    <div class="summary-small nav">
        <!-- <a href="./"><img src="../public/img/logo.png" class="logo-small" alt="logo" /></a></li> -->
        <a href="./" class="home-nav-small">ACCUEIL</a>
        <a href="../public/index.php?route=takeAway" class="carte-emporter-small">LA CARTE À EMPORTER</a>
        <a href="../public/img/carte-pdf.pdf" class="carte-place-small">LA CARTE SUR PLACE</a>
        <a href="../public/index.php?route=contact" class="contact-nav-small">CONTACT</a>
    </div>

    <script type="text/javascript" src="../public/js/responsiveSummary.js"></script>

    <nav class="summary">
        <ul>
            <li><a href="./"><img src="../public/img/logo.png" class="logo" alt="logo" /></a></li>

            <li class="menu-nav-li">
                <a href="#" class="menu-nav">LA CARTE &ensp;</a>
                <ul class="sous">
                    <li><a href="../public/index.php?route=takeAway" class="carte-emporter">À EMPORTER</a></li>
                    <li><a href="../public/img/carte-pdf.pdf" class="carte-place">SUR PLACE</a></li>
                </ul>
            </li>

            <li class="home-nav-li"><a href="../public/index.php" class="home-nav"><i class="fas fa-home"></i></a></li>
            <li class="contact-nav-li"><a href="../public/index.php?route=contact" class="contact-nav">CONTACT</a></li>
        </ul>

        <div class="icones-nav">
            <div class="click-nav-li">
                <a href="https://papacionuparis.byclickeat.fr/order?fbclid=IwAR3i-74lj-cwhRtYQYjZutW0KTdYPFfO_gEK1uXBlDmk_Cu9WWIQ9Dq1IXQ" target="_blank" class="click-nav">Click & Collect</a>
            </div>
            <div class=phone-number-bloc>
                <a href="tel:+33145239003" class="phone-number-nav"><i class="fas fa-mobile-alt"></i><span id="infobulle-tel">01.45.23.90.03</span></a>
            </div>
            <a href="mailto:Papacionuaemporter@gmail.com" class="mail-papacionu-nav" target="_blank"><i class="far fa-envelope"></i></a>
            <a href="https://www.facebook.com/Papacionu" class="fb-link-nav" target="_blank"><i class="fab fa-facebook-f"></i></a>
            <a href="https://www.instagram.com/papacionuparis/?hl=fr" class="insta-link-nav" target="_blank"><i class="fab fa-instagram"></i></a>
        </div>
    </nav>
</header>